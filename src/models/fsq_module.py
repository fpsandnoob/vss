...
from src.models.components.fsq_model_utils import FSQLPIPSWithDiscriminator


class FSQVAEModelLitModule(LightningModule):
    ...
    def __init__(
        ...
        num_vq_embeddings: int = 8*8*8*6*5,
    ):
        super().__init__()
        ...
        # loss function
        self.loss = FSQLPIPSWithDiscriminator(
            disc_start=11000,
    ...
    def forward(self, x: torch.Tensor, return_pred_indices=False):
        ...
        if return_pred_indices:
            return output.sample, output.indices
    ...
    def training_step(self, batch, batch_idx):
        ...
        xrec, ind = self(x, return_pred_indices=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        xrec, ind = self(x, return_pred_indices=True)
        aeloss, log_dict_ae = self.loss(
                                        x.view(-1, 1, *x.shape[-2:]), xrec.view(-1, 1, *x.shape[-2:]), 
                                        0,
                                        self.global_step,
                                        last_layer=self.get_last_layer(),
                                        split="val",
        ...
        if batch_idx == 0:
            original_grid_images = make_grid(x.view(-1, 1, *x.shape[-2:]), nrow=8, normalize=True, range=(-1, 1))
            self.logger.experiment.add_image(
                "val/original",
                original_grid_images,
                self.current_epoch
            )
            reconstructed_grid_images = make_grid(xrec.view(-1, 1, *x.shape[-2:]), nrow=8, normalize=True, range=(-1, 1))
            self.logger.experiment.add_image(
                "val/reconstructed",
