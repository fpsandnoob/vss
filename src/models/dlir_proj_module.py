...
class EvalModule(LightningModule):
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        # gt = batch
        proj, metadata = batch
        measurement = (proj.clone() * degrade_op.vox_scaling)[:, ::degrade_op.downsample_factor][None]
        
        gt = degrade_op.gt_backproject(proj.float().clone() * degrade_op.vox_scaling)[None]
        # gt = degrade_op.backproject(proj.clone() * degrade_op.vox_scaling)[None]
        gt_hu = 1000 * ((gt - degrade_op.hu_factor) / degrade_op.hu_factor)
        print(gt_hu.max(), gt_hu.min())
        gt = gt_hu.clip(-2048, 3200)
        print(gt.max(), gt.min())
        gt = (gt - (-2048)) / (3200 - (-2048))
        print(gt.max(), gt.min())
        
        # measurement = degrade_op.forward(batch)
        # print(batch.max(), batch.min())
        # print(measurement.max(), measurement.min(), measurement.shape)
        ...
        for gt_im in gt:
            ...
            # print(gt_im.shape)
            # plt.imsave(os.path.join(self.im_out_dir, f"{batch_idx}_wgt_plt.png"), get_window(gt_im.permute(1, 2, 0).squeeze()).detach().cpu().numpy(), cmap='gray', vmax=0.2122, vmin=0.1951)
        ...
        print(os.path.join(self.im_out_dir, f"{batch_idx}.png"), " saved!")
        plt.close()
