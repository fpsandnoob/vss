...
from .measurement_ops import NonLinearOp, LinearOp, OpCompose, NoiseOp, SuperResolutionOp, CTSparseViewOp
...
class LatentDeepLatentIterativeReconstructEnsembler(LatentGuidenceModule):
    ...
    def grad_and_diff(self, l_prev, l_0_hat, l_t, measurement, **kwargs):
        ...
        l_0_hat = repeat(l_0_hat, "b c h w -> (b k) c h w", k=k)
        l_0_hat = l_0_hat + torch.randn_like(l_0_hat) * (variance ** (0.5))
        # l_0_hat = l_0_hat.clamp(1, -1)
        
        x_0_hat = self.vae.decode(l_0_hat).sample
        deg_x_0_hat = self.degrade_op.forward(x_0_hat)
        deg_x_0_hat_repeat = rearrange(deg_x_0_hat, "(b k) c h w -> b c h w k", k=k)
        
        measurement_repeat = repeat(measurement, "b c h w -> b c h w k", k=k)
        diff_val = torch.linalg.vector_norm(deg_x_0_hat_repeat - measurement_repeat, ord=1, dim=-1)
        diff_val = torch.exp(diff_val)
        diff_val = torch.mean(diff_val)
        diff_val = torch.log(diff_val)
        # print(diff_val)
