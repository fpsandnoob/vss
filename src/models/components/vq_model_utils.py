...
class LPIPSwithpixelloss(nn.Module):
    def __init__(
        self,
        pixelloss_weight=1.0,
        perceptual_weight=1.0,
    ...
    def forward(self, inputs, reconstructions, split="train"):
        # print(inputs.max(), inputs.min(), reconstructions.max(), reconstructions.min())
        inputs = inputs / 2 + 0.5
        reconstructions = reconstructions / 2 + 0.5
        inputs = inputs.float()
        reconstructions = reconstructions.float()
        # print(inputs.max(), inputs.min(), reconstructions.max(), reconstructions.min())
        pixel_loss = self.pixel_loss(inputs.contiguous(), reconstructions.contiguous())
        if self.perceptual_weight > 0:
            p_loss = self.perceptual_loss(
                inputs.contiguous(), reconstructions.contiguous()
            )
        ...
        rec_loss = pixel_loss + self.perceptual_weight * p_loss
        # print(pixel_loss.mean(), p_loss.mean())
        nll_loss = rec_loss
        # nll_loss = torch.sum(nll_loss) / nll_loss.shape[0]
        nll_loss = torch.mean(nll_loss)
